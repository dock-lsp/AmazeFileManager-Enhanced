...
class OpenFileDialogFragment : BaseBottomSheetFragment(), AdjustListViewForTv<AppHolder> {
    ...
    companion object {
        ...
        fun openFileOrShow(
            ...
            // Check if this is a media file that should be handled by built-in player
            if (!forceChooser && isMediaFile(mimeType)) {
                openWithBuiltInPlayer(uri, mimeType, useNewStack, activity)
                return
            }
        ...
        /**
         * Check if mime type is a media file (audio or video)
         */
        ...
        /**
         * Open media file with built-in player
         */
        private fun openWithBuiltInPlayer(
            uri: Uri,
            mimeType: String,
            useNewStack: Boolean,
            activity: PreferenceActivity,
            ...
            val intent = Intent(activity, MediaPlayerActivity::class.java).apply {
                data = uri
                putExtra(MediaPlayerActivity.EXTRA_FILE_URI, uri)
                putExtra(MediaPlayerActivity.EXTRA_MIME_TYPE, mimeType)
                putExtra(MediaPlayerActivity.EXTRA_IS_VIDEO, mimeType.startsWith("video/"))
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
                if (useNewStack && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
